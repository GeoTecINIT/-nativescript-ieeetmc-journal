...
In both cases #2 group, encompasses all the instructions being run between the storage of the `planning_timestamp` of the next alarm and the storage of the `execution_timestamp` of the actual task. NTD encompasses a lot more instructions than the ad-hoc application.
...
overhead_clean %>%
  ...
  dplyr::summarise(task_execution_delay_min = min(task_execution_delay),
                   ...
                   task_execution_delay_q3 = quantile(task_execution_delay, .75),
                   task_execution_delay_max = max(task_execution_delay),
                   task_execution_delay_sd = sd(task_execution_delay)) -> overhead_summaries_scheduler
...
battery_data %>%
  group_by(plan_month, plan_day, plan_hour, device_id, scheduler) %>%
  arrange(step) %>%
...
head_bin %>%
  ...
  ggplot2::facet_wrap(device_id ~ scheduler)
...
head_bin %>%
  arrange(bin) %>%
  mutate(battery_offset = abs(battery - lead(battery))) %>%
  ggplot(aes(x=bin, y=battery_offset, color=device_id)) +
  ggplot2::labs(title = "Battery offset (last - first reading) per hour (1 hour = 1 bin)") +
  ggplot2::geom_line(size = 0.5) +
  ggplot2::facet_wrap(device_id ~ scheduler)
