...
data_path <- here::here("exp1", "data", "data_ieeetmc.csv")
write_csv(data_complete, data_path)
data_path <- here::here("exp1", "data", "data_ieeetmc.rds")
...
assessment_file <- here::here("exp1", "data", "data_ieeetmc.rds")
...
## Figure 4 (included in the paper)
...
Based on one comment from a reviewer, we compute a new variable, **overhead**, as the difference between `exec_timestamp(n) - planning_timestamp(n+1)`. Due to the fact that `planning_timestamp(n+1)` is computed in the `n` task execution, this computation can reflect the overhead that the NTD introduces in the process caused by the complexity of the task planning and execution process based on alarm trigger. This complexity depends on the instructions set executed in each solution according to: 
...
In both cases #2 group, encompasses all the instructions being run between the storage of the `planning_timestamp` of the next alarm and the storage of the `execution_timestamp` of the actual task. NTD encompases a lot more instructions than the ad-hoc application.
...
overhead %>%
  ...
  filter(task_execution_delay > 0) %>%
  filter(task_execution_delay < 2000) -> overhead_clean
...
overhead_clean %>%
  ...
  dplyr::summarise(task_execution_delay_min = min(task_execution_delay),
                   ...
                   task_execution_delay_sd = sd(task_execution_delay)) -> overhead_summaries

knitr::kable(overhead_summaries,
             format = "html",
             booktabs = TRUE,
             caption = "TABLE. Overhead comparison (unit: milliseconds)") %>%
  kableExtra::kable_styling(full_width = TRUE, bootstrap_options = c("condensed"))
...
overhead_clean %>%
  ...
  dplyr::summarise(task_execution_delay_min = min(task_execution_delay),
                   ...
                   task_execution_delay_q3 = quantile(task_execution_delay, .75),
                   task_execution_delay_max = max(task_execution_delay),
                   task_execution_delay_sd = sd(task_execution_delay)) -> overhead_summaries_per_scheduler

knitr::kable(overhead_summaries_per_scheduler,
             format = "html",
             booktabs = TRUE,
             caption = "TABLE. Overhead per scheduler comparison (unit: milliseconds)") %>%
  kableExtra::kable_styling(full_width = TRUE, bootstrap_options = c("condensed"))
```
