...
Based on one comment from a reviewer, we compute a new variable, **overhead**, as the difference between consecutive `plan_date`. 


```{r overhead_dataprep, warning=FALSE}

data %>%
  filter(device_id %in% c("H9", "BQ")) %>%
  select(-exec_timestamp, -planning_timestamp) -> overhead
...
overhead <-
  ...
  group_by(device_id, scheduler) %>%
  arrange(plan_date) %>%
  mutate(overhead_diff = plan_date - dplyr::lag(plan_date),
         overhead_diff_secs = as.numeric(overhead_diff, units = "secs"),
         overhead_delay = overhead_diff_secs - baseline_delay)


overhead %>%
  tidyr::drop_na(overhead_delay) %>%
  dplyr::group_by(device_id, scheduler) %>%
  dplyr::summarise(overhead_min = min(overhead_delay),
                   overhead_q1 = quantile(overhead_delay, .25, ),
                   overhead_median = median(overhead_delay),
                   overhead_q3 = quantile(overhead_delay, .75),
                   overhead_max = max(overhead_delay),
                   overhead_sd = sd(overhead_delay)) -> overhead_summaries
...
knitr::kable(overhead_summaries,
             format = "html",
             booktabs = TRUE,
             caption = "TABLE. Overhead comparison (unit: seconds)") %>%
  kableExtra::kable_styling(full_width = TRUE, bootstrap_options = c("condensed"))
