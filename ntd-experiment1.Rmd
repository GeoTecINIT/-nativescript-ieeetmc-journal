...
We look here if energy consumption is a barrier for the NTD application, compared to the less resource-demanding ad-hoc application. Again, we focues here only on the BQ and H9 devices that successfully completed the experiment.
...
head_bin %>%
  arrange(bin) %>%
  mutate(battery_offset = battery - lead(battery)) %>%
  filter(battery_offset >= 0, device_id == "BQ") %>%
  group_by(device_id, scheduler) %>%
  summarise(mean_offset = mean(battery_offset, na.rm=T))
