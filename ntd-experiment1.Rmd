...
head_bin_bq %>%
  ...
  ggplot2::labs(title = "Battery offset (last - first reading) computed by hour (1 hour = 1 bin)") +
  ggplot2::geom_line(size = 0.5) +
...
head_bin_bq %>%
  ...
  ggplot(aes(x=bin, y=battery_offset, color=scheduler)) +
  ggplot2::labs(title = "Battery offset (last - first reading) > 0 computed by hour (1 hour = 1 bin)") +
  ggplot2::geom_line(size = 0.5) +
  ggplot2::facet_wrap(~ scheduler)
...
knitr::kable(t(head_bin_bq_summaries),
             format = "html",
             booktabs = TRUE,
             caption = "TABLE. BQ' battery usage offset") %>%
  kableExtra::kable_styling(full_width = TRUE, bootstrap_options = c("condensed"))
