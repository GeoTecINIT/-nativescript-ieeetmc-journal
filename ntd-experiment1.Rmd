...
### Overhead comparison per scheduler and device
...
### Overhead comparison per scheduler
...
Here, we compute the BQ's battery usage offset (last - fist battery reading) within a bin (1 bin = 1 hour).
...
Here, we zoom in the BQ's battery usage offset greater than zero to discard bins in whch battery is being charged. This corresponds to Figure 5 in the paper.
...
head_bin_bq %>%
  ...
  summarise(
    ...
    offet_sd = round(sd(battery_offset, na.rm=T), 2)) -> head_bin_bq_summaries
