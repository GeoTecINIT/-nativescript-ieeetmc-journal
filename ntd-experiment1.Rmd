...
Based on one comment from a reviewer, we compute a new variable, **overhead**, as the difference between consecutive `exec_timestamp(n) - planning_timestamp(n+1)`. Due to the fact that `planning_timestamp(n+1)` is computed in the `n` task execution, this computation can reflect the overhead that the NTD introduces in the process caused by its complexity.
